...
import com.zmn.biz.amislc.aspect.log.anno.LogPrintIgnore;
import com.zmn.biz.amislc.aspect.log.anno.LogPrintIgnoreInputParam;
import com.zmn.biz.amislc.aspect.log.anno.LogPrintIgnoreOutputResponse;
...
public abstract class BaseLogAspect {
        ...
        try {
            ...
            if (Objects.nonNull(persistentLogConsumer)) {
                ...
                if (Objects.nonNull(persistentLog)) {
                    persistentLogConsumer.consumePersistentLog(envType, targetClazz, persistentLog, inputParamMap, outputResponse);
                    break outIf;
                }
                // 没有持久化注解，但方法名以[add]开头，自动记入添加
                boolean matches = methodName.startsWith("add");
                if (matches) {
                    persistentLogConsumer.consumePersistentLog(envType, targetClazz, ADD_LP, inputParamMap, outputResponse);
                    break outIf;
                }
                // 没有持久化注解，但方法名以[modify]开头，自动记入修改
                matches = methodName.startsWith("modify");
                if (matches) {
                    persistentLogConsumer.consumePersistentLog(envType, targetClazz, MODIFY_LP, inputParamMap, outputResponse);
                    break outIf;
                }
                // 没有持久化注解，但方法名以[delete]开头，自动记入删除
                matches = methodName.startsWith("delete");
                if (matches) {
                    persistentLogConsumer.consumePersistentLog(envType, targetClazz, DELETE_LP, inputParamMap, outputResponse);
                }
            }
        }
    }
    ...
    private Map<String, Object> buildParamMap(Method method, Object[] args, String operator) {
        ...
        for (int i = 0, len = params.length; i < len; i++) {
            ...
            if (arg instanceof BaseModifyDIO) {
                // 自动填充操作人
